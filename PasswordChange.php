...
if (isset($_SESSION['Email']) &&!empty($_SESSION['Email'])) {
    ...
    if (isset($_POST['SecAnswer'])) {
        if ($answer === $_POST['SecAnswer']) {
            ...
            if ($_POST['Password'] === $_POST['Confirm_password']) {
                echo "<script>alert('Success, password is now being reset.');window.location='Account.php'</script>";
                $sql_update = mysqli_query($conn, "Update user set password = '$hash' where email = '$sessionemail'");
            //If passwords entered do not match, alert user to try again
            } else {
                echo "<script>alert('Error, both passwords entered did not match. Please try again.');window.location='PasswordChange.php'</script>";
            }
        //Alert user that security answer did not match
        } else {
    ...
    echo "<script>alert('Please set an email.');window.location='ForgotPassword.php'</script>";
}
?>